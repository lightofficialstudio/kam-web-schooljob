...
// ✨ Schema เปลี่ยนรหัสผ่าน
const changePasswordSchema = z.object({
  new_password: z.string().min(8, "รหัสผ่านต้องมีอย่างน้อย 8 ตัวอักษร"),
});

// ✨ PUT /api/v1/employer/account-setting/change-password?user_id=xxx
export async function PUT(request: Request) {
  try {
    ...
    if (!parsed.success) {
      return Response.json(
        { status_code: 400, message_th: parsed.error.errors[0]?.message || "ข้อมูลไม่ถูกต้อง", message_en: "Invalid input", data: null },
        { status: 400 },
      );
    }

    // ✨ ตรวจว่า profile มีอยู่จริง
    ...
    // ✨ เปลี่ยนรหัสผ่านผ่าน Supabase Admin API (service role)
    ...
    if (error) {
      ...
      return Response.json(
        { status_code: 400, message_th: "เปลี่ยนรหัสผ่านไม่สำเร็จ", message_en: error.message, data: null },
        { status: 400 },
      );
    }
    ...
    console.error("❌ [account-setting/change-password]:", err);
    return Response.json(
