...
import { createWorkExperienceService } from "./service/work-experience-service";

// ✨ POST /api/v1/employee/work-experiences?user_id=xxx — เพิ่มประวัติการทำงานใหม่
export async function POST(request: Request) {
  try {
    ...
    const result = await createWorkExperienceService(userId, parsed.data);

    return Response.json(
      {
        status_code: 201,
        message_th: "เพิ่มประวัติการทำงานสำเร็จ",
        message_en: "Work experience created",
        data: result,
      },
      { status: 201 }
    );
    ...
    console.error("❌ [POST /api/v1/employee/work-experiences]:", error);
    return Response.json(
      {
        status_code: 500,
